...
import ActionMenu from '@/pages/v3/service/dashboard/search/ActionMenu';
  ...
  const columns: ColumnProps<IService>[] = [
    {
      title: 'Name',
      key: 'name',
      render: (row) => <Link to={`/v3/service/${row._id}`}>{row.name}</Link>,
    },
    {
      title: 'Vendor',
      key: 'vendor',
      render: (row) => <Link to={`/v3/vendor/${row.vendor._id}`}>{row.vendor.name}</Link>,
    },
    {
      title: 'Vendor price',
