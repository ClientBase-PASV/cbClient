...
import {
  ...
  queryUserGetById, queryUserVerifyEmailSend,
} from '@/pages/v2/user/queries';
  ...
  effects: {
    ...
    *login({ payload }, { call, put }) {
      ...
      if (name && token && userId) {
        ...
        if (!emailConfirmed) history.push('/v2/onboarding');
        else history.push('/v2/client');
      }
    },
    ...
    *logout(_, { put }) {
      ...
      history.push('/v2/user/login');
    },

    *passwordReset({ payload }, { call }) {
      yield call(queryUserPasswordReset, payload);
      history.push('/v2/user/password/reset/mailed');
    },
