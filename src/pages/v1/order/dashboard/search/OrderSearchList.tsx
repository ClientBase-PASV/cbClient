...
import ActionMenu from '@/pages/v1/order/dashboard/search/ActionMenu';
  ...
  const columns: ColumnProps<IOrder>[] = [
    {
      title: 'Order',
      key: 'name',
      render: (row) => <Link to={`/v1/order/${row._id}`}>{row.name}</Link>,
    },
    {
      title: 'Client',
      key: 'client',
      render: (row) => <Link to={`/v1/client/${row.client_id}`}>{row.client.name}</Link>,
    },
    {
      title: 'clientPrice',
